({
    getEvaluationFormDataFromUrl: function (component, event, helper) {
        ...
        var companyName = helper.getUrlParameter("nc").replace(/xyzamperandzyx/g, '&');
        var language = helper.getUrlParameter("language");
        component.set('v.currentLanguage', language);
        ...
        companyName = companyName.replace(/hardpluslup/g, '+').replace(/dhiaesshe/g, '#').replace(/dfgcerpentgfd/g,'%').replace(/dfgmaqrkgfd/g,'?');
            ...
            action.setCallback(
                ...
                $A.getCallback(function (response) {
                    ...
                    if (state == "SUCCESS") {
                        ...
                        if(formData.currentEvaluation.Statut__c == 'Évaluée' || formData.currentEvaluation.CanEditReadError__c == 'Read') {
                            component.set('v.isReadOnly', true);
                            helper.hideSpinner(component);
                        }
                        var formEditionTiming = formData.currentEvaluation.CanEditReadError__c;
                        if(formEditionTiming != '' && (formEditionTiming != 'Edit' && formEditionTiming != 'Read' )) {
                            helper.showErrorPopup(component, event, formEditionTiming);
                            helper.hideSpinner(component);
                            return;
                        }
                        component.set("v.formData", formData);
                        component.set("v.noData", formData);
    ...
    checkEarlyMakerCompetencesAndComment: function(component, nbEarlyMaker, nbRatedEarlyMaker) {
        ...
        if ((competenceEarlyMakerAverageRating <= 2 || competenceEarlyMakerAverageRating=="n/a") && commentaire_EarlyMaker__c == "") {
            component.set('v.isEarlyMakerCompetencesEvalDone', false);
    ...
    checkComportementCompetencesAndComment: function(component, nbComport, nbRatedComport) {
        ...
        if ((competenceComportementAverageRating <= 2 ||competenceComportementAverageRating =="n/a") && commentaire_Comportementales__c == "") {
    ...
    checkMetiersFamilyAndCompetencesAndComment: function(component, nbMetier, nbRatedMetier) {
        ...
        if ((competenceMetiersAverageRating <= 2 || competenceMetiersAverageRating == "n/a") && commentaire_Metiers__c == "") {
    ...
    checkGlobalEvaluationSection: function(component) {
        ...
        if (commentaire_global__c == "" || !commentaire_global__c ) {
            component.set('v.isGlobalEvalDone', false);
